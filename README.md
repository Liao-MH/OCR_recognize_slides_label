...
- 默认额外生成一个运行日志文件，记录批处理执行过程和错误细节
- 默认在终端实时显示处理进度，并在失败时立即输出错误摘要
...
程序还会默认输出一个运行日志文件，路径是 `output_csv` 同目录下的 `*.run.log`。这个日志文件记录批处理启动、逐 slide 处理进度、失败 traceback、预览生成状态和结束汇总，适合在大批量失败时排查问题。

程序运行时还会默认在终端实时显示进度，正常情况下用单行覆盖显示当前处理到的 slide；一旦某个 slide 失败，会立刻在终端打印一条错误摘要，而完整 traceback 仍保留在 `*.run.log` 中。
...
11. 默认写出 `output_csv` 同目录下的 `*.run.log` 运行日志文件
12. 默认在终端实时显示处理进度，并即时输出逐文件失败摘要
13. 在终端输出本次运行的总数、成功数、失败数以及输出文件路径

## 错误处理原则

- 对明显非法的输入目录，直接显式报错
- 对单个 SVS 的处理错误，隔离到该文件，不影响整个批次
- 默认运行日志会保留逐文件处理过程和失败 traceback，便于排查批量失败原因
- 默认终端输出只显示实时进度和失败摘要，不打印完整 traceback
